...
public class CacheDownloader {
	...
	private void connect() {
		...
		//requester.connect("localhost", 1, -1, "");
		
		while (requester.getState() != CacheRequester.State.CONNECTED) {
			requester.process();
		}
		System.out.println("Successful connection");
		File dir = new File("./data/"+CacheRequester.major+"");
		dir.mkdir();
	}
	...
	private void initCacheIndices(int count) {
		try {
			RandomAccessFile dataFile = new RandomAccessFile("./data/"+CacheRequester.major+"/main_file_cache.dat2", "rw");
			RandomAccessFile referenceFile = new RandomAccessFile("./data/"+CacheRequester.major+"/main_file_cache.idx255", "rw");
			reference = new FileStore(255, dataFile.getChannel(), referenceFile.getChannel(), 0x7a120);
			System.out.println(count);
			stores = new FileStore[count];
			for (int i = 0; i < count; i++) {
				RandomAccessFile indexFile = new RandomAccessFile("./data/"+CacheRequester.major+"/main_file_cache.idx" + i, "rw");
